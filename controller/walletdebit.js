...
const WalletDebit = models.WalletDebit;
var moment = require("moment");

const create = async (req, res) => {
  try {
    const data = req.body;
    await WalletDebit.create(data)
      .then((data) => {
        res.json({
          status: 200,
          message: "SUCCESS",
...
const viewByid = async (req, res) => {
  try {
    ...
    await WalletDebit.findAll({ where: { id: data } })
      .then((data) => {
        res.send(data);
      })
      .catch((err) => {
...
const update = async (req, res) => {
  try {
    ...
    await WalletDebit.update(value, {
      where: {
        id: id,
      },
    })
...
const destroy = async (req, res) => {
  try {
    const data = req.body.id;
    await WalletDebit.destroy({ where: { id: data } })
      .then(() => {
